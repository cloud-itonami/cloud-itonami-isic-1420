# cloud-itonami-isic-1420

Open Business Blueprint for **ISIC Rev.5 1420**: manufacture of articles
of fur -- fur-garment and fur-accessory manufacturing (cutting, matching
and sewing lines) that turns **already-dressed and dyed fur pelts** into
finished coats, trims and accessories. Dressing and dyeing of fur itself
is a distinct upstream ISIC class (tanning and dressing of leather;
dressing and dyeing of fur) and is explicitly **out of scope** here -- this
repository only covers the downstream garment/accessory-manufacturing
step, paired with
[cloud-itonami-isic-1410](https://github.com/cloud-itonami/cloud-itonami-isic-1410)
(wearing apparel, except fur apparel) as the sibling apparel vertical in
the 衣食住 scaffold batch (ADR-2607122200).

## What the implemented actor is

**Fur-Garment Operations Advisor ⊣ Fur Governor** -- the fleet-standard
pattern: the advisor LLM drafts production-batch logging (cutting/sewing
batch, output-quality data), cutting/sewing-line-equipment maintenance
scheduling, safety/quality-concern flags, and outbound finished-garment
shipment coordination; the independent `:fur-governor` (a keyword unique
fleet-wide) gates every action; physical-domain work (cutting, matching,
sewing, pressing, packing) is executed by plant engineers/robots under
...
> jurisdiction's fur-labeling/species-sourcing requirements are official,
> no license to actually log a real production batch or coordinate a real
> shipment, and no way to know on its own whether the plant/batch it is
> acting on has actually been independently verified**. Letting it commit
> a batch record or coordinate a shipment directly invites fabricated
> spec-basis citations, an unverified plant/batch being silently recorded,
> and a genuine equipment-safety or labeling concern being quietly logged
> instead of escalated. This project seals the advisor into a single node
> and wraps it with an independent **Fur Governor**, a human approval
...
fur-garment/accessory manufacturing plant operations. It does **not**:

- Operate cutting equipment or sewing machines
- Make design decisions about patterns, cuts, or pelt-matching
- Control production-line parameters (speed, tension, nailing/blocking
  pressure, etc.)
- Approve pelt or dressing quality (that is the upstream dresser's
  responsibility, not this plant's)
- Dress or dye fur pelts (a distinct upstream ISIC class)
...
with a keyword scan covering both generic cutting/sewing-line terms and
fur-specific pelt-handling terms (nailing, blocking, stretching a pelt
onto a board during letting-out/matching).
...
(`fur.registry/allowed-ops`) that both the registry drafters and the
governor's `op-not-allowlisted-violations` check require, so the two can
never drift out of sync:

| Op | Purpose |
|---|---|
| `:proposal/log-production-batch` | Log a completed cutting/sewing batch and its output-quality data |
| `:proposal/schedule-maintenance` | Propose cutting/sewing-line-equipment maintenance scheduling |
| `:proposal/flag-safety-concern` | Surface an equipment-safety or output-quality-defect concern -- **ALWAYS escalates** |
| `:actuation/coordinate-shipment` | Coordinate outbound finished-garment shipment |
...
```
plant/batch record + jurisdiction facts (fur.facts, spec-cited)
        |
        v
   ┌──────────────┐   proposal      ┌────────────────────────┐
   │ Fur-Garment  │ ─────────────▶ │ Fur Governor:            │  (independent system)
...
**The advisor never logs a batch or coordinates a shipment the Fur
...
`fur.facts/coverage` reports how many requested jurisdictions actually
have an official spec-basis in `fur.facts/catalog` -- currently 3 seeded
(USA, Italy, Canada) out of ~194 jurisdictions worldwide. This is a
starting catalog to prove the governor contract end-to-end, not a claim
of global coverage. Every citation is a real, verifiable official source
(never fabricated):

| Jurisdiction | Requirement | Spec-basis |
|---|---|---|
| USA | Fur-garment labeling | Fur Products Labeling Act, 15 U.S.C. § 69; FTC Fur Rules, 16 CFR Part 301 |
| USA | Species sourcing (CITES) | Endangered Species Act, 16 U.S.C. § 1538; CITES implementing regulations, 50 CFR Part 23 |
| USA | Labor standards | FLSA 29 CFR § 516, OSHA 1910 Subpart A |
| Italy | Non-textile animal-origin labeling | Regulation (EU) No 1007/2011, Article 12 |
...
| Canada | Fur-content labeling | Textile Labelling Act, R.S.C. 1985, c. T-10; Textile Labelling and Advertising Regulations, C.R.C., c. 1551 |
| Canada | Labour standards | Canada Labour Code, R.S.C. 1985, c. L-2 |

Adding a jurisdiction is additive: one map entry in `fur.facts/catalog`,
...
`1420`). This vertical's plant/batch records are practice-specific rather
than a shared cross-operator data contract, so `fur.*` runs on the
...
sewing station under `kotoba-lang/robotics` safety classes) performs the
actual cutting, matching and sewing under this actor, gated by the
independent **Fur Governor**. The governor never operates equipment
...
| Layer | What is open |
|---|---|
| OSS core | Actor runtime, Fur Governor, batch/maintenance/shipment draft records, audit ledger |
...
`:implemented` -- `Fur-Garment Operations Advisor` + `Fur Governor` run as
real, tested code (see `Run` above), modeled closely on
`cloud-itonami-isic-1410`'s verified module shape.

## License

Code and implementation templates are AGPL-3.0-or-later, forkable by any
qualified operator, so local fur-garment makers never surrender
