(ns fur.registry
  "Proposal registry and drafting helpers for fur-garment manufacturing plant
  operations. Every proposal carries its spec-basis and evidence checklist.")
...
(def allowed-ops
  ...
  `fur.governor`'s `op-not-allowlisted-violations`, which requires this same
  def (rather than re-declaring its own copy) so the two can never drift out
  of sync."
...
(defn batch-log-draft
  "Draft a production batch logging proposal (cutting/sewing batch,
  output-quality data).
  subject: batch ID
...
(defn maintenance-draft
  "Draft a cutting/sewing-line-equipment maintenance scheduling proposal.
...
(defn shipment-draft
  "Draft an outbound finished-garment shipment coordination proposal
