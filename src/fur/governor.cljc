(ns fur.governor
  "Fur-Garment Manufacturing Plant Operations Governor -- the independent
  compliance layer that earns the Fur-Garment Operations Advisor the right
  to propose and log actions. The LLM has no notion of species-sourcing
  law, garment-labeling regulation, or when a shipment or batch-logging is
  a real-world actuation, so this MUST be a separate system able to
  *reject* a proposal and fall back to HOLD.
  ...
  HARD violations (a human approver CANNOT override):
    ...
    6. Direct equipment control -- NO cutting/sewing-line operation (those
                                 remain plant-engineer exclusive)
  ...
  This actor coordinates PLANT OPERATIONS around fur-garment/accessory
  manufacturing FROM ALREADY-DRESSED PELTS -- dressing and dyeing of fur is
  a distinct upstream ISIC class and out of scope here. It does NOT:
...
(def process-control-keywords
  ...
  general cutting/sewing-line terms plus fur-specific pelt-handling terms
  (nailing/blocking/stretching a pelt onto a board during letting-out and
  matching)."
  #{"speed" "tension" "needle" "presser" "feed" "stitch" "pattern"
    "cutting" "sewing" "operate" "control" "blade" "angle" "parameter"
    "thread" "adjust" "trim" "nailing" "blocking" "stretching"})
...
(defn- op-not-allowlisted-violations
  "HARD: :op must be a member of the CLOSED `fur.registry/allowed-ops`
...
(defn- process-control-block-violations
  "HARD BLOCK: This actor does NOT operate cutting/sewing-line equipment.
  If a proposal mentions cutting speed, sewing tension, needle control,
  pelt nailing/blocking/stretching, or other process parameters, reject it
  immediately. Those decisions remain the exclusive authority of licensed
