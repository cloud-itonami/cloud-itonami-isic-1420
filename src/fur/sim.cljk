(ns fur.sim
  "Simulation harness for Fur-Garment Manufacturing Plant Operations
  Coordinator actor.
  Run with: clojure -M:dev:run"
  (:require [fur.advisor :as advisor]
            [fur.governor :as governor]
            [fur.store :as store]))

(defn -main
  "Drive a simple fur-garment manufacturing workflow through the governor."
  [& _args]
  (let [st (store/mem-store)
    ...
    (println "=== FUR-GARMENT MANUFACTURING PLANT OPERATIONS COORDINATOR SIMULATION ===\n")
