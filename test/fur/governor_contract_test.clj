(ns fur.governor-contract-test
  (:require [clojure.test :refer [deftest is]]
            [fur.store :as store]
            [fur.advisor :as advisor]
            [fur.governor :as governor]
            [fur.registry :as registry]))
...
(deftest ^{:doc "HARD BLOCK: Proposals mentioning cutting speed, sewing tension, pelt
  nailing/blocking, or other equipment-control terms are immediately
  rejected. Those remain plant-engineer exclusive authority."} process-control-block
  (let [st (store/mem-store)
...
(deftest ^{:doc "HARD BLOCK: fur-specific pelt-handling process terms (nailing a pelt
  onto a stretching board during letting-out/matching) are also forbidden
  equipment-control language, not just generic cutting/sewing terms."} process-control-block-fur-specific-terms
  (let [st (store/mem-store)
        proposal {:op :proposal/log-production-batch
                  :subject "batch-001"
                  :effect :propose
                  ...
                  :value {:evidence {:batch-verified true}
                          :confidence 0.9
                          :detail "Please adjust the pelt nailing and blocking tension on the board"}}]
    (let [eval (governor/evaluate proposal st)]
      (is (:holds? eval) "Fur-specific process-control proposal should hold")
      (is (some #(= (:rule %) :process-control-forbidden) (:hard-violations eval))
        "Should have process-control-forbidden violation"))))
  ...
  (let [st (store/mem-store)
        proposal {:op :proposal/flag-safety-concern
                  ...
                  :value {:evidence {:concern-documented true}
                          ...
                          :detail "Non-textile animal origin disclosure missing on shipment"}}]
    (let [eval (governor/evaluate proposal st)]
...
(deftest ^{:doc "HARD: An :op outside the closed `fur.registry/allowed-ops` set is
