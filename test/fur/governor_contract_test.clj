...
(deftest process-control-block
  "HARD BLOCK: Proposals mentioning cutting speed, sewing tension, pelt
  nailing/blocking, or other equipment-control terms are immediately
  rejected. Those remain plant-engineer exclusive authority."
  (let [st (store/mem-store)
...
(deftest process-control-block-fur-specific-terms
  "HARD BLOCK: fur-specific pelt-handling process terms (nailing a pelt
  onto a stretching board during letting-out/matching) are also forbidden
  equipment-control language, not just generic cutting/sewing terms."
  (let [st (store/mem-store)
        proposal {:op :proposal/log-production-batch
                  :subject "batch-001"
                  :effect :propose
...
(deftest op-not-allowlisted-blocks
  "HARD: An :op outside the closed `fur.registry/allowed-ops` set is
